...
int main()
{
	...
	//Лысачев сказал собственно работу сервера (этот цикл while) вынести в отдельную нить/процесс, а главная нить будет в бесконечном цикле спрашивать у пользователя команды (информация о сервере, завершение работы и т.п.)
	//При получении команды завершения, главная нить загоняет в пайп задания с finish == true, а также завершает этот while (видимо, в нити будет не while(1), а while(!finish), 
	//где finish - ссылка на общий с главной нитью bool, которому после получения команды выставляем значение true) и ожидает все нити
    while(1)
    {
